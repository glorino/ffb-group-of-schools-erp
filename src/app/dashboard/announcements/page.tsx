...
function AnnouncementsPageInner() {
  ...
  const [activeTab, setActiveTab] = useState<"announcements" | "news" | "events">(
    (searchParams?.get("tab") as "announcements" | "news" | "events") || "announcements"
  );
  ...
  const newsItems = filteredAnnouncements.filter(a => a.type === "news");
  const eventItems = filteredAnnouncements.filter(a => a.type === "event");
        ...
        >
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-white/40">
              ...
              <p className="text-[13px]">No news posts yet. Click "New News/Event" to create one.</p>
            </div>
          ) : (
        ...
        >
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-white/40">
              ...
              <p className="text-[13px]">No events yet. Click "New News/Event" to create one.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {eventItems.map((item) => {
                const target = item.target || {};
                return (
                  <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition-all">
                    <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-emerald-500/30 to-emerald-600/30 flex flex-col items-center justify-center shrink-0">
                      {target.eventDate ? (
                        <>
                          <span className="text-emerald-400 text-[18px] font-bold leading-none">{new Date(target.eventDate).getDate()}</span>
                          <span className="text-emerald-400/70 text-[10px] uppercase">{new Date(target.eventDate).toLocaleDateString("en", { month: "short" })}</span>
                        </>
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-white font-medium text-[13px] mb-0.5">{item.title}</h4>
                      <p className="text-white/50 text-[12px] line-clamp-1">{item.content}</p>
                      {target.eventDate && (
                        <span className="text-emerald-400 text-[11px]">{new Date(target.eventDate).toLocaleDateString("en", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</span>
                      )}
                    </div>
