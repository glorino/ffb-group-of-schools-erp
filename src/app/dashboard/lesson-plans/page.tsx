          ...
          {filtered.length === 0 ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[#1a1a2e] text-[13px] font-medium">{lp.topic}</p>
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  <span className="text-[#64748b] text-[11px]">{lp.subject}</span>
                  <span className="text-[#94a3b8]">·</span>
                  <span className="text-[#64748b] text-[11px]">{lp.className}</span>
                  <span className="text-[#94a3b8]">·</span>
                  <span className="text-[#64748b] text-[11px]">{lp.teacher.firstName} {lp.teacher.lastName}</span>
                  <span className="text-[#94a3b8]">·</span>
              ...
              <div className="flex items-center gap-1">
                ...
                {lp.status === "pending" && (
                  <button onClick={() => handleStatusUpdate(lp.id, "approved")} className="p-2 rounded-lg hover:bg-emerald-500/10 text-[#94a3b8] hover:text-[#16a34a] transition text-[10px]">Approve</button>
                )}
                <button onClick={() => handleDelete(lp.id)} className="p-2 rounded-lg hover:bg-red-500/10 text-[#94a3b8] hover:text-[#dc2626] transition"><Trash2 className="w-4 h-4" /></button>
              </div>
            </motion.div>
          ))}
        </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0]">
                <div>
                  <h3 className="text-[#1a1a2e] font-semibold">{showDetail.topic}</h3>
                  <p className="text-[#64748b] text-[12px] mt-0.5">{showDetail.subject} · {showDetail.className}</p>
                </div>
                <button onClick={() => setShowDetail(null)} className="text-[#64748b] hover:text-[#475569] transition"><X className="w-5 h-5" /></button>
              </div>
              <div className="px-6 py-5 space-y-4">
                ...
                {showDetail.objectives && (
                  <div>
                    <p className="text-[#64748b] text-[11px] uppercase tracking-wider font-medium mb-1">Objectives</p>
                    <p className="text-[#475569] text-[13px]">{showDetail.objectives}</p>
                  </div>
                )}
                <div>
                  <p className="text-[#64748b] text-[11px] uppercase tracking-wider font-medium mb-1">Content</p>
                  <p className="text-[#475569] text-[13px] whitespace-pre-wrap">{showDetail.content}</p>
                </div>
                {showDetail.resources && (
                  <div>
                    <p className="text-[#64748b] text-[11px] uppercase tracking-wider font-medium mb-1">Resources</p>
                    <p className="text-[#475569] text-[13px]">{showDetail.resources}</p>
                  </div>
                )}
              </div>
            </motion.div>
