...
export default function DisciplinePage() {
  ...
  const typeColors: Record<string, string> = { behavior: "bg-amber-500/20 text-amber-400", academic: "bg-blue-500/20 text-blue-400", attendance: "bg-red-500/20 text-red-400", safety: "bg-purple-500/20 text-purple-400" };
  const actionColors: Record<string, string> = { pending: "bg-yellow-500/20 text-yellow-400", warning: "bg-amber-500/20 text-amber-400", suspension: "bg-red-500/20 text-red-400", expulsion: "bg-red-700/20 text-red-300", resolved: "bg-green-500/20 text-green-400" };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm">Track and manage student discipline records</p>
        </div>
            ...
            <tbody>
              {filteredRecords.map(r => (
                <tr key={r.id} className="border-b border-white/5 hover:bg-white/5">
                  <td className="py-3 px-4 text-white">{r.student.firstName} {r.student.lastName}</td>
                  ...
                  <td className="py-3 px-4 text-white/70">{r.title}</td>
                  <td className="py-3 px-4 text-white/50">{new Date(r.date).toLocaleDateString()}</td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <button onClick={() => { setEditingRecord(r); setForm({ studentId: r.studentId, type: r.type, title: r.title, details: r.details || "", date: new Date(r.date).toISOString().split("T")[0], action: r.action || "pending" }); setShowModal(true); }} className="text-blue-400 hover:text-blue-300 mr-3">Edit</button>
                    <button onClick={() => handleDelete(r.id)} className="text-red-400 hover:text-red-300">Delete</button>
                  </td>
                </tr>
              ))}
              {filteredRecords.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-white/30">{loading ? "Loading..." : "No discipline records found"}</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <AnimatePresence>
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={e => e.stopPropagation()} className="bg-[#0f1b33] border border-white/10 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <h2 className="text-xl font-bold text-white mb-4">{editingRecord ? "Edit Record" : "New Discipline Record"}</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-white/50 text-sm mb-1">Student</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-white/50 text-sm mb-1">Action</label>
                ...
                <div>
                  <label className="block text-white/50 text-sm mb-1">Details</label>
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-2 border border-white/10 rounded-lg text-white/70 hover:bg-white/5">Cancel</button>
