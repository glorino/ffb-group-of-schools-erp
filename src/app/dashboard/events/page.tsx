  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-white text-2xl font-bold">Events Management</h1>
          <p className="text-white/50 text-sm mt-1">Create and manage school events and activities</p>
        </div>
        {canManage && (
          <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-sm font-medium hover:shadow-lg hover:shadow-emerald-500/25 transition-all">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-white font-semibold text-lg">All Events</h3>
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-[13px] focus:outline-none focus:border-emerald-500"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-start gap-4 p-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition-all">
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-emerald-500/30 to-emerald-600/30 flex flex-col items-center justify-center shrink-0">
                    <span className="text-emerald-400 text-[18px] font-bold leading-none">{day}</span>
                    <span className="text-emerald-400/70 text-[10px] uppercase">{month}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-white font-medium text-[14px] truncate">{item.title}</h4>
                      {item.target?.featured && (
                        <span className="px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-400 text-[11px] font-medium shrink-0">Featured</span>
                      )}
                    </div>
                    <p className="text-white/50 text-[13px] line-clamp-1 mb-1">{item.content}</p>
                    {eventDate && (
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-lg rounded-2xl bg-[#0a1628] border border-white/[0.12] shadow-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-white font-semibold text-lg">{editItem ? "Edit Event" : "New Event"}</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-[13px] focus:outline-none focus:border-emerald-500" placeholder="Enter event title" />
              </div>
              <div>
                <label className="block text-white/60 text-[13px] mb-1.5">Content *</label>
                <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={3} className="w-full px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-[13px] focus:outline-none focus:border-emerald-500 resize-none" placeholder="Write event description..." />
              </div>
              <div>
                <label className="block text-white/60 text-[13px] mb-1.5">Event Date *</label>
                <input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-[13px] focus:outline-none focus:border-emerald-500" style={{ colorScheme: "dark" }} />
              </div>
              <div>
                <label className="block text-white/60 text-[13px] mb-1.5">Event Image</label>
                <div className="flex gap-2">
                  <input type="text" value={imageUrl} onChange={(e) => { setImageUrl(e.target.value); setImageFile(""); }} placeholder="Image URL" className="flex-1 px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-[13px] focus:outline-none focus:border-emerald-500" />
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-sm font-medium hover:shadow-lg hover:shadow-emerald-500/25 disabled:opacity-50 flex items-center gap-2">
